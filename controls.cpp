...
#if 1 // AWH
        /* Player one joystick? */
        if ((id & 0xFF000000) == 0x80000000)
        {
          /* Snag the player one start/select button mappings for the GUI */
          if (mapping.button.joypad.buttons == SNES_SELECT_MASK) {
            selectButtonNum = id & 0xFF;
          } else if (mapping.button.joypad.buttons == SNES_START_MASK) {
            startButtonNum = id & 0xFF;
          }
        }
#endif // AWH
	if (t >= 0)
	...
	switch (cmd.type)
	{
		...
		case S9xButtonJoypad:
			...
			else
			{
				...
				if (data1)
				{
					if (!Settings.UpAndDown /* AWH - BeagleSNES && !S9xMoviePlaying()*/) // if up+down isn't allowed AND we are NOT playing a movie,
					{
